...
from scripts import build_cn_search_core_v2_production_wave4_prefreeze as builder
from scripts import run_cn_search_core_v2_production_wave4_v1 as runner
from our_system_phase2.services.program_search_state_jump_generator_v2 import SEMANTIC_STATE_JUMP_GENERATOR_V2

REPO = Path(__file__).resolve().parents[1]


def test_production_wave4_prefreeze_is_single_arm_pair_annotated_and_zero_financial():
    payload = builder.build(REPO, source_repo_sha="a" * 40)
    assert payload["status"] == builder.STATUS
    assert payload["production_wave4"]["arm"] == SEMANTIC_STATE_JUMP_GENERATOR_V2
    assert payload["production_wave4"]["templates"] == [
        "BASE_EVENT",
        "BASE_MARKET_EVENT",
        "BASE_TEMPORAL_EVENT",
        "BASE_TEMPORAL_MARKET_EVENT",
    ]
    assert payload["production_wave4"]["held_templates"] == ["BASE_MARKET", "BASE_TEMPORAL", "BASE_TEMPORAL_MARKET"]
    assert payload["production_wave4"]["total_financial_evaluations"] == 192
    assert payload["production_wave4"]["total_checkpoint_count"] == 8
    assert payload["mature_state"]["source_development_observations"] == 1920
    assert payload["mature_state"]["expected_final_development_observations"] == 2112
...
def test_production_wave4_execution_plan_exact():
    plan = runner._execution_plan()
    assert len(plan) == 8
    assert sum(row["batch_size"] for row in plan) == 192
    assert {row["production_round_index"] for row in plan} == {0, 1}
    assert {row["template_id"] for row in plan} == set(runner.TEMPLATES)
