...
from scripts import build_cn_search_core_v2_production_wave3_prefreeze as builder
from scripts import run_cn_search_core_v2_production_wave3_v1 as runner
from our_system_phase2.services.program_search_state_jump_generator_v2 import SEMANTIC_STATE_JUMP_GENERATOR_V2

REPO = Path(__file__).resolve().parents[1]


def test_production_wave3_prefreeze_is_single_arm_pair_annotated_and_zero_financial():
    payload = builder.build(REPO, source_repo_sha="a" * 40)
    assert payload["status"] == builder.STATUS
    assert payload["production_wave3"]["arm"] == SEMANTIC_STATE_JUMP_GENERATOR_V2
    assert payload["production_wave3"]["templates"] == [
        "BASE_EVENT",
        "BASE_MARKET_EVENT",
        "BASE_TEMPORAL_EVENT",
    ...
    assert payload["production_wave3"]["held_templates"] == ["BASE_MARKET", "BASE_TEMPORAL"]
    assert payload["production_wave3"]["total_financial_evaluations"] == 240
    assert payload["production_wave3"]["total_checkpoint_count"] == 10
    assert payload["mature_state"]["source_development_observations"] == 1680
    assert payload["mature_state"]["expected_final_development_observations"] == 1920
...
def test_production_wave3_execution_plan_exact():
    plan = runner._execution_plan()
    assert len(plan) == 10
    assert sum(row["batch_size"] for row in plan) == 240
