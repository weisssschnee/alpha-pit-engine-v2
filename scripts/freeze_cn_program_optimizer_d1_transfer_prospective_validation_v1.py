"""Freeze a completed fresh D1 development cohort for prospective transfer validation.

All development-productive Programs are frozen before validation access.  The
already-frozen transfer filter is applied first and its top-40% membership is
recorded, but every development-positive Program remains in the report-only
validation cohort so filter precision and recall can be measured prospectively.
"""
...
from scripts.apply_cn_program_optimizer_d1_transfer_filter_v1 import apply_filter
...
def freeze(*, run_root: Path, filter_path: Path, output_root: Path, source_cohort: str) -> dict[str,Any]:
    run_root=run_root.resolve(); output_root=output_root.resolve()
    if output_root.exists(): raise FileExistsError(output_root)
    closure_path=run_root/D1_CLOSURE
    closure=_read_json(closure_path); body=dict(closure); claimed=str(body.pop("closure_payload_sha256",""))
    if not claimed or stable_hash(body)!=claimed or closure.get("status")!="CN_PROGRAM_OPTIMIZER_D1_DEVELOPMENT_COMPLETE": raise RuntimeError("D1 continuation closure drift")
    if int(closure.get("closed_waves") or 0)!=20 or int(closure.get("logical_records") or 0)!=140 or int(closure.get("physical_evaluation_calls") or 0)!=140: raise RuntimeError("D1 continuation closure cardinality drift")
    if any(int(v)!=0 for v in (closure.get("restricted_reads") or {}).values()): raise RuntimeError("D1 continuation restricted read drift")
    ...
    for wave in range(20):
        wr=run_root/f"wave_{wave:03d}"
        manifest_path=wr/"wave_manifest.json"; manifest_sha=_sha256(manifest_path)
        ...
        for exact, filt in by_exact.items():
            ...
            if ask is None or schedule is None or result is None: raise RuntimeError(f"D1 continuation freeze lineage missing: {exact}")
            credit=dict((result.get("uplift") or {}).get("program_credit") or {})
            if not bool(result["admission"]["admitted"]) or float(credit.get("matched_cumulative_net_return_increment") or 0)<=0 or float(credit.get("matched_net_reward_increment") or 0)<=0: raise RuntimeError("transfer application includes non-productive Program")
            members.append({
                ...
                "development_robust_median_window_return_increment":float(filt["dev_robust_median"]),"development_lower_tail_window_return_increment":float(filt["dev_lower_tail"]),
                "transfer_linear_score":float(filt["transfer_linear_score"]),"transfer_probability_score":float(filt["transfer_probability_score"]),"transfer_filter_selected":bool(filt["transfer_filter_selected"]),
            })
    members.sort(key=lambda r:str(r["exact_identity"]))
    if len(members)!=int(application["development_productive_count"]) or len({m["exact_identity"] for m in members})!=len(members): raise RuntimeError("D1 prospective freeze cardinality drift")
    selected=sorted(m["exact_identity"] for m in members if m["transfer_filter_selected"])
    if len(selected)!=int(application["selected_count"]): raise RuntimeError("D1 prospective filter selected count drift")
    output_root.mkdir(parents=True, exist_ok=True)
    members_path=_write_jsonl(output_root/"validation_candidate_members.jsonl",members)
    freeze_payload={
    ...
    freeze_path=_write_json(output_root/"validation_candidate_freeze.json",freeze_payload)
...
def main(argv:Sequence[str]|None=None)->int:
    p=argparse.ArgumentParser(description=__doc__); p.add_argument("--run-root",type=Path,required=True); p.add_argument("--filter",type=Path,required=True); p.add_argument("--output-root",type=Path,required=True); p.add_argument("--source-cohort",default="D1_CONTINUATION_B"); a=p.parse_args(argv)
    result=freeze(run_root=a.run_root,filter_path=a.filter,output_root=a.output_root,source_cohort=a.source_cohort)
    print(json.dumps({k:result[k] for k in ("status","candidate_count","transfer_filter_selected_count","candidate_exact_identities_sha256","freeze_payload_sha256","freeze_file_sha256")},sort_keys=True))
    return 0
