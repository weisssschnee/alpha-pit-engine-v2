"""Build Production Wave 2 authorization after zero-financial supply/canary."""
...
def build(repo: Path, *, canary: Path, supply_audit: Path) -> dict[str, Any]:
    ...
    supply_hash = _verify(supply, "audit_payload_sha256", "Production Wave 2 supply")
    canary_path = canary.resolve()
    canary_payload = _read(canary_path)
    canary_hash = _verify(canary_payload, "official_canary_payload_sha256", "Production Wave 2 canary")
    runner = repo / "scripts/run_cn_search_core_v2_production_wave2_v1.py"
    ...
    if (
        ...
        raise RuntimeError("Production Wave 2 policy not approved")
    pair_contract = dict(prefreeze.get("pair_native_annotation_contract") or {})
    if (
        prefreeze.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE2_PREFROZEN_BEFORE_FINANCIAL_READ"
        or int(prefreeze["production_wave2"]["total_financial_evaluations"]) != runtime.TOTAL_EVALUATIONS
        ...
        raise RuntimeError("Production Wave 2 prefreeze drift")
    if (
        ...
        raise RuntimeError("Production Wave 2 supply drift")
    if (
        ...
        raise RuntimeError("Production Wave 2 canary drift")
    payload = {
        "schema_version": runtime.AUTHORIZATION_SCHEMA,
        "status": "SEARCH_CORE_V2_PRODUCTION_WAVE2_AUTHORIZED_NOT_RUN",
        ...
        "production_contract": dict(prefreeze["production_wave2"]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = build(args.repo_root, canary=args.official_resource_canary, supply_audit=args.production_wave2_supply_audit)
