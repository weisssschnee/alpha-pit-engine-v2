...
def _acceptance_contract(flt:Mapping[str,Any])->dict[str,Any]:
 keys=[k for k in ("prospective_B_validation_acceptance","prospective_C_validation_acceptance") if k in flt]
 if len(keys)!=1: raise RuntimeError(f"prospective transfer acceptance contract cardinality drift: {keys}")
 return dict(flt[keys[0]])
...
def build(*,repo_root:Path,freeze_path:Path,prepared_path:Path,source_contract:Path,registry:Path,output:Path,transfer_filter_path:Path|None=None)->dict[str,Any]:
 ...
 filter_path=(transfer_filter_path.resolve() if transfer_filter_path is not None else (root/FILTER_RELATIVE_PATH).resolve()); flt=_read(filter_path); _verify(flt,"filter_payload_sha256","transfer filter")
 allowed_filter_status={"FROZEN_BEFORE_NEXT_DEVELOPMENT_COHORT_VALIDATION","FROZEN_BEFORE_C_DEVELOPMENT_AND_VALIDATION"}
 if freeze.get("status")!="FROZEN_BEFORE_VALIDATION_ACCESS" or prepared.get("status")!="D1_TRANSFER_PROSPECTIVE_VALIDATION_PREFINANCIAL_READY" or flt.get("status") not in allowed_filter_status: raise RuntimeError("prospective validation input status drift")
 ...
 if str(prepared.get("candidate_members_payload_sha256") or "")!=str(freeze["candidate_members_payload_sha256"]): raise RuntimeError("prospective prepared members semantic drift")
 payload={
...
def main(argv:Sequence[str]|None=None)->int:
 p=argparse.ArgumentParser(description=__doc__); p.add_argument("--repo-root",type=Path,required=True); p.add_argument("--freeze",type=Path,required=True); p.add_argument("--prepared",type=Path,required=True); p.add_argument("--transfer-filter",type=Path); p.add_argument("--source-contract",type=Path,required=True); p.add_argument("--registry",type=Path,required=True); p.add_argument("--output",type=Path,required=True); a=p.parse_args(argv)
 result=build(repo_root=a.repo_root,freeze_path=a.freeze,prepared_path=a.prepared,source_contract=a.source_contract,registry=a.registry,output=a.output,transfer_filter_path=a.transfer_filter); print(json.dumps(result,ensure_ascii=False,sort_keys=True)); return 0
