...
def build(*,repo_root:Path,freeze_path:Path,preflight_path:Path,source_contract:Path,registry:Path,output:Path,transfer_filter_path:Path,development_outcome_path:Path,reproducibility_audit_path:Path)->dict[str,Any]:
 ...
 if int(preflight.get("validation_reads_by_this_preflight") if preflight.get("validation_reads_by_this_preflight") is not None else -1)!=0 or bool(preflight.get("candidate_evaluation_executed")): raise RuntimeError("prospective preflight crossed validation boundary")
