"""Zero-financial first-checkpoint supply audit for Production Wave 2."""
...
from scripts import run_cn_search_core_v2_production_wave2_v1 as wave
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    prefreeze = wave.verify_prefreeze(args.production_wave2_prefreeze.resolve())
    provisional = {
        "authorization_payload_sha256": stable_hash(
            {"role": "PRODUCTION_WAVE2_ZERO_FINANCIAL_SUPPLY", "prefreeze": prefreeze["prefreeze_payload_sha256"]}
        ),
    ...
    if engine._sha256(state_path) != str(prefreeze["mature_state"]["file_sha256"]):
        raise RuntimeError("PRODUCTION_WAVE2_SUPPLY_SOURCE_FILE_DRIFT")
    snapshot = _read(state_path)
    source_hash = _verify(snapshot, "snapshot_hash", "Production Wave 2 source state")
    if source_hash != str(prefreeze["mature_state"]["payload_sha256"]):
        raise RuntimeError("PRODUCTION_WAVE2_SUPPLY_SOURCE_HASH_DRIFT")
    ...
    for ordinal, template in enumerate(stage1.TEMPLATES):
        ...
        rows = stage2._generated_schedules_batch(
            ...
            checkpoint_id=f"PRODUCTION_WAVE2_SUPPLY_{template}",
    ...
    if (
        ...
        raise RuntimeError("PRODUCTION_WAVE2_SUPPLY_EXACT_DRIFT")
    fields = tuple(sorted(map(str, engine._checkpoint_field_columns(schedules))))
    if not fields:
        raise RuntimeError("PRODUCTION_WAVE2_SUPPLY_FIELD_SURFACE_EMPTY")
    payload = {
        "schema_version": "cn_search_core_v2_production_wave2_supply_audit_v1",
        "status": "PASS_ZERO_FINANCIAL_PRODUCTION_WAVE2_CHECKPOINT_SUPPLY_AUDIT",
