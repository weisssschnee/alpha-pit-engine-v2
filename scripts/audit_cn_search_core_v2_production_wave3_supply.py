"""Zero-financial first-checkpoint supply audit for Production Wave 3."""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    provisional = {
        "authorization_payload_sha256": stable_hash(
            {"role": "PRODUCTION_WAVE3_ZERO_FINANCIAL_SUPPLY", "prefreeze": prefreeze["prefreeze_payload_sha256"]}
        ),
    ...
    if engine._sha256(state_path) != str(prefreeze["mature_state"]["file_sha256"]):
        raise RuntimeError("PRODUCTION_WAVE3_SUPPLY_SOURCE_FILE_DRIFT")
    snapshot = _read(state_path)
    source_hash = _verify(snapshot, "snapshot_hash", "Production Wave 3 source state")
    if source_hash != str(prefreeze["mature_state"]["payload_sha256"]):
        raise RuntimeError("PRODUCTION_WAVE3_SUPPLY_SOURCE_HASH_DRIFT")
    ...
    for ordinal, template in enumerate(wave.TEMPLATES):
        ...
        rows = stage2._generated_schedules_batch(
            ...
            checkpoint_id=f"PRODUCTION_WAVE3_SUPPLY_{template}",
    ...
    if (
        ...
        raise RuntimeError("PRODUCTION_WAVE3_SUPPLY_EXACT_DRIFT")
    fields = tuple(sorted(map(str, engine._checkpoint_field_columns(schedules))))
    if not fields:
        raise RuntimeError("PRODUCTION_WAVE3_SUPPLY_FIELD_SURFACE_EMPTY")
    payload = {
        "schema_version": "cn_search_core_v2_production_wave3_supply_audit_v1",
        "status": "PASS_ZERO_FINANCIAL_PRODUCTION_WAVE3_CHECKPOINT_SUPPLY_AUDIT",
