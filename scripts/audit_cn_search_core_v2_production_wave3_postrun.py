"""Post-run audit for Search Core V2 Production Wave 3."""
...
from scripts import run_cn_search_core_v2_production_wave3_v1 as wave3
from our_system_phase2.services.unified_capability_registry import stable_hash

STATUS = "SEARCH_CORE_V2_PRODUCTION_WAVE3_POSTRUN_AUDIT_COMPLETE_DEVELOPMENT_ARCHIVE_READY"
...
def audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    final_state_hash = _verify(final_state, "snapshot_hash", "Production Wave 3 final optimizer state")
    prefreeze = _read(repo / "runtime/run_plans/cn_search_core_v2_production_wave3_prefreeze_20260826.json")
    prefreeze_hash = _verify(prefreeze, "prefreeze_payload_sha256", "Production Wave 3 prefreeze")
    supply = _read(repo / "runtime/run_plans/cn_search_core_v2_production_wave3_supply_audit_20260826.json")
    supply_hash = _verify(supply, "audit_payload_sha256", "Production Wave 3 supply audit")
    canary = _read(repo / "runtime/run_plans/cn_search_core_v2_production_wave3_official_resource_canary_20260826.json")
    canary_hash = _verify(canary, "official_canary_payload_sha256", "Production Wave 3 canary")
    authorization = _read(repo / "runtime/run_plans/cn_search_core_v2_production_wave3_authorization_v1.json")
    authorization_hash = _verify(authorization, "authorization_payload_sha256", "Production Wave 3 authorization")
    source_state_path = repo / Path(str(prefreeze["mature_state"]["relative_path"]))
    source_state = _read(source_state_path)
    source_state_hash = _verify(source_state, "snapshot_hash", "Production Wave 3 source mature state")
    ...
    if (
        terminal.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE3_COMPLETE"
        or int(terminal.get("evaluated") or 0) != 240
        or int(terminal.get("checkpoint_count") or 0) != 10
        ...
        or int(terminal.get("source_mature_observations") or 0) != 1680
        or int(terminal.get("final_mature_observations") or 0) != 1920
        ...
        raise RuntimeError("Production Wave 3 terminal contract drift")
    ...
    if (
        ...
        raise RuntimeError("Production Wave 3 pair-native terminal contract drift")
    if (
        supply.get("status") != "PASS_ZERO_FINANCIAL_PRODUCTION_WAVE3_CHECKPOINT_SUPPLY_AUDIT"
        or int(supply.get("generated_total") or 0) != 120
        ...
        or authorization.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE3_AUTHORIZED_NOT_RUN"
    ):
        raise RuntimeError("Production Wave 3 prefinancial authority drift")

    checkpoint_dirs = [root / f"checkpoint_{ordinal:04d}" for ordinal in range(10)]
    if any(not path.is_dir() for path in checkpoint_dirs):
        raise RuntimeError("Production Wave 3 missing closed checkpoint")
    if list(root.glob("checkpoint_*.inflight")):
        raise RuntimeError("Production Wave 3 has inflight checkpoint after terminal")
    ...
    for ordinal, checkpoint in enumerate(checkpoint_dirs):
        ...
        if (
            ...
            raise RuntimeError("Production Wave 3 checkpoint chain drift")
        for artifact in list(manifest.get("artifacts") or ()):
            ...
            if not artifact_path.is_file() or _sha(artifact_path) != str(artifact["sha256"]):
                raise RuntimeError("Production Wave 3 checkpoint artifact drift")
        previous = _sha(manifest_path)
        rows = _read_jsonl(checkpoint / "candidate_results.jsonl")
        if len(rows) != 24:
            raise RuntimeError("Production Wave 3 checkpoint result count drift")
        for row in rows:
            _verify(row, "result_payload_sha256", f"checkpoint {ordinal} result")
            if row.get("production_wave_id") != "SEARCH_CORE_V2_PRODUCTION_WAVE3_V1":
                raise RuntimeError("Production Wave 3 result identity drift")
            ...
            if bool(annotation.get("available")):
                ...
                if (
                    ...
                    raise RuntimeError("Production Wave 3 pair-native annotation recomputation drift")
            elif bool(annotation.get("challenger_hit")):
                raise RuntimeError("Production Wave 3 unavailable pair marked challenger")
        ...
        if "pair_native" in tell_text or "challenger" in tell_text:
            raise RuntimeError("Production Wave 3 pair-native feedback leaked into optimizer tell")
    ...
    if previous != str(terminal["last_checkpoint_manifest_file_sha256"]):
        raise RuntimeError("Production Wave 3 terminal checkpoint chain drift")
    if len(all_rows) != 240 or len({str(row["exact_identity"]) for row in all_rows}) != 240:
        raise RuntimeError("Production Wave 3 result exact coverage drift")
    active_templates = tuple(map(str, prefreeze["production_wave3"]["templates"]))
    held_templates = tuple(map(str, prefreeze["production_wave3"]["held_templates"]))
    observed_templates = {str(row["template_id"]) for row in all_rows}
    if tuple(active_templates) != tuple(wave3.TEMPLATES) or observed_templates != set(active_templates) or observed_templates & set(held_templates):
        raise RuntimeError("Production Wave 3 focused template budget drift")

    recomputed_overall = stage1._metric(all_rows)
    if recomputed_overall != dict(terminal["overall_metrics"]):
        raise RuntimeError("Production Wave 3 overall metric drift")
    for template in wave3.TEMPLATES:
        ...
        if metric != dict(terminal["per_template_metrics"][template]):
            raise RuntimeError("Production Wave 3 per-template metric drift")
    for round_index in range(2):
        ...
        if metric != dict(terminal["round_metrics"][str(round_index)]):
            raise RuntimeError("Production Wave 3 round metric drift")
    ...
    if (
        len(source_history) != 70
        or len(final_history) != 80
        or final_history[:70] != source_history
        or len(source_generated) != 1680
        or len(final_generated) != 1920
        or len(new_generated) != 240
        or new_generated != result_exacts
        or len(new_generated & source_seen) != 0
        or int(final_history[-1]["generator_diagnostics"]["memory_observations"]) != 1920
    ):
        raise RuntimeError("Production Wave 3 mature state lineage drift")
    ...
    if (
        ...
        raise RuntimeError("Production Wave 3 candidate archive drift")
    ...
    if (
        ...
        raise RuntimeError("Production Wave 3 pair-native challenger archive drift")
    for ordinal, metric in enumerate(checkpoint_metrics):
        ...
        if (
            ...
            raise RuntimeError("Production Wave 3 checkpoint pair-native metric drift")
    ...
    if (
        ...
        raise RuntimeError("Production Wave 3 execution terminal state drift")

    payload = {
        "schema_version": "cn_search_core_v2_production_wave3_postrun_audit_v1",
        "status": STATUS,
        ...
        "mature_state_lineage": {
            ...
            "source_history_count": 70,
            "final_history_count": 80,
            "source_generated_exact_count": 1680,
            "final_generated_exact_count": 1920,
            "new_generated_exact_count": 240,
            "new_generated_vs_source_seen_overlap_count": 0,
            "final_memory_observations": 1920,
        },
        "development_yield": {
            "evaluated": 240,
            ...
            "per_template": {template: dict(terminal["per_template_metrics"][template]) for template in wave3.TEMPLATES},
        ...
        "project_control_recommendation": {
            "verdict": "HOLD_WAVE3_DEVELOPMENT_ARCHIVE_AND_PAIR_CHALLENGER_NO_AUTOMATIC_VALIDATION",
            "next_action": "FREEZE_WAVE3_DEVELOPMENT_EVIDENCE_AND_COMPARE_WAVE1_WAVE2_WAVE3_YIELD_WITHOUT_READING_VALIDATION_OR_OOS",
