...
CAMPAIGN_ID='CN_PROGRAM_PRIMITIVE_MARKET_SUCCESSOR_V2'
CAMPAIGN_PROFILE='cn_program_primitive_market_successor_v2'
STATUS_COMPLETE='PRIMITIVE_MARKET_SUCCESSOR_V2_COMPLETE'
CLOSURE_NAME='CN_PROGRAM_PRIMITIVE_MARKET_SUCCESSOR_V2_COMPLETE.json'
CHECKPOINT_SIZE=24
PLAN_RELATIVE_PATH=Path('runtime/run_plans/cn_program_primitive_market_successor_v2_plan.json')
...
def verify_plan(path:Path)->dict[str,Any]:
 ...
 if not claim or stable_hash(body)!=claim or p.get('status')!='PRIMITIVE_MARKET_SUCCESSOR_V2_PLAN_FROZEN_NOT_RUN' or p.get('candidate_evaluation_executed') is not False: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_PLAN_DRIFT')
 b=dict(p['budget']); f=dict(p['fresh_supply']); a=dict(p['search_authority'])
 if int(b['hard_cap_logical_records'])!=360 or int(b['checkpoint_count'])!=15 or int(b['checkpoint_size'])!=24 or int(f['effective_spent_exact_count'])!=7934 or int(f['fresh_unique_count'])!=3139: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_PLAN_GEOMETRY_DRIFT')
 if a.get('production_feedback_imported_into_primitive_stats') is not False or a.get('v1_successor_feedback_imported_into_primitive_stats') is not False or int(a.get('production_results_in_stats') or 0)!=0 or int(a.get('v1_successor_results_in_stats') or 0)!=0 or a.get('diversity_contract_changed') is not False or int(a.get('max_variants_per_base_per_template') or 0)!=4: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_V2_SEARCH_AUTHORITY_DRIFT')
 rc=dict(p['resource_contract']); root=Path(__file__).resolve().parents[1]; aa=_verify_acceleration_accuracy_audit(p,root); _verify_v1_evidence(p,root); g=dict(p['prospective_gate'])
 if rc.get('profile')!='SEARCH_DUAL_24' or int(rc.get('primary_executor_workers') or 0)!=24 or int(rc.get('fallback_executor_workers') or 0)!=16 or rc.get('evaluator_pool_lifetime')!='PERSISTENT_RUN_SCOPE' or float(rc.get('minimum_records_per_hour_after_first_checkpoint') or 0)<650.0 or int(rc.get('minimum_free_memory_bytes') or 0)<24*1024**3 or int(rc.get('throughput_enforcement_after_warm_checkpoints') or 0)!=2 or rc.get('persistent_pool_record_hash_parity_required') is not True : raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_RESOURCE_AUDIT_DRIFT')
 if float(g.get('core_primitive_uplift_stable_2of3_rate_min') or 0)!=0.20 or float(g.get('core_primitive_to_combined_core_controls_uplift_stable_rate_ratio_min') or 0)!=1.50 or float(g.get('each_core_template_primitive_uplift_stable_2of3_rate_min') or 0)!=0.15: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_STABLE_GATE_DRIFT')
...
def _verify_v1_evidence(plan:Mapping[str,Any],repo_root:Path)->dict[str,Any]:
 tb=dict(plan['source_v1_terminal_audit']);tp=(repo_root/Path(str(tb['relative_path']))).resolve()
 if not tp.is_relative_to(repo_root.resolve()) or not tp.is_file() or hashlib.sha256(tp.read_bytes()).hexdigest()!=str(tb['file_sha256']): raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_V2_V1_AUDIT_FILE_DRIFT')
 tr=_read(tp);body=dict(tr);claim=str(body.pop('audit_payload_sha256',''))
 if claim!=str(tb['payload_sha256']) or stable_hash(body)!=claim or tr.get('status')!='PASS_INDEPENDENT_TERMINAL_AUDIT' or int(tr['counts']['records'])!=360 or int(tr['counts']['selected_unique'])!=360 or tr.get('oos_authority')!='NONE': raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_V2_V1_AUDIT_PAYLOAD_DRIFT')
 pb=dict(plan['source_v1_acceleration_full_parity']);pp=(repo_root/Path(str(pb['relative_path']))).resolve()
 if not pp.is_relative_to(repo_root.resolve()) or not pp.is_file() or hashlib.sha256(pp.read_bytes()).hexdigest()!=str(pb['file_sha256']): raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_V2_PARITY_FILE_DRIFT')
 pr=_read(pp)
 if str(pr.get('audit_payload_sha256'))!=str(pb['payload_sha256']) or pr.get('status')!='PASS_FULL_360_ECONOMIC_PARITY' or int(pr.get('economic_drift_count',-1))!=0 or pr.get('accelerated_run_classification')!='DIAGNOSTIC_PARITY_REPLAY_ONLY' or pr.get('selected_set_reused_for_search_evidence') is not False or pb.get('accelerated_run_search_evidence_authorized') is not False: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_V2_PARITY_PAYLOAD_DRIFT')
 return {'v1_terminal_audit':tr,'v1_acceleration_parity':pr}
...
def _fresh_catalog(plan:Mapping[str,Any],authority:Mapping[str,Any],repo_root:Path)->dict[str,Any]:
 ...
 if claim!=bind['payload_sha256'] or stable_hash(body)!=claim or supply.get('status')!='ZERO_FINANCIAL_PRIMITIVE_MARKET_SUCCESSOR_V2_FRESH_SUPPLY_READY' or int(supply['fresh_unique_count'])!=3139 or int(supply['effective_spent_exact_count'])!=7934: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_FRESH_SUPPLY_DRIFT')
 ...
 if len(entries)!=3139 or set(catalog_by_exact)!={e.exact_identity for e in entries} or len(set(physical_by_normalized.values()))!=3139: raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_V2_CATALOG_COVERAGE_DRIFT')
...
def _gate(plan:Mapping[str,Any],feedback:Sequence[Mapping[str,Any]],records:Sequence[Mapping[str,Any]])->dict[str,Any]:
 ...
 bp=large._metric_block(corep); bc=large._metric_block(controls); bsp=_stable_block(corep); bsc=_stable_block(controls); total=large._metric_block(feedback); per={t:large._metric_block([r for r in corep if str(r.get('template_id'))==t]) for t in sorted(CORE)}; per_stable={t:_stable_block([r for r in corep if str(r.get('template_id'))==t]) for t in sorted(CORE)}; br=_behavior_count(records)/len(records) if records else 0.0; control_rate=float(bc['productive_efficiency']); primitive_rate=float(bp['productive_efficiency']); ratio=(primitive_rate/control_rate) if control_rate>0 else (999.0 if primitive_rate>0 else 0.0); stable_control_rate=float(bsc['uplift_stable_2of3_rate']);stable_primitive_rate=float(bsp['uplift_stable_2of3_rate']);stable_ratio=(stable_primitive_rate/stable_control_rate) if stable_control_rate>0 else (999.0 if stable_primitive_rate>0 else 0.0)
 checks={'core_primitive_productive_rate':float(bp['productive_efficiency'])>=float(g['core_primitive_productive_rate_min']),'core_primitive_vs_controls_ratio':ratio>=float(g['core_primitive_to_combined_core_controls_rate_ratio_min']),'each_core_template':all(float(v['productive_efficiency'])>=float(g['each_core_template_primitive_productive_rate_min']) for v in per.values()),'core_primitive_uplift_stable_2of3_rate':stable_primitive_rate>=float(g['core_primitive_uplift_stable_2of3_rate_min']),'core_primitive_vs_controls_uplift_stable_ratio':stable_ratio>=float(g['core_primitive_to_combined_core_controls_uplift_stable_rate_ratio_min']),'each_core_template_uplift_stable':all(float(v['uplift_stable_2of3_rate'])>=float(g['each_core_template_primitive_uplift_stable_2of3_rate_min']) for v in per_stable.values()),'total_productive':int(total['productive'])>=int(g['total_productive_count_min']),'behavior_pair_rate':br>=float(g['behavior_pair_rate_min'])}
 return {'status':'PRIMITIVE_MARKET_SUCCESSOR_V2_TRANSFER_PASS' if all(checks.values()) else 'PRIMITIVE_MARKET_SUCCESSOR_V2_TRANSFER_FAIL','checks':checks,'core_primitive':bp,'combined_core_controls':bc,'core_rate_ratio_vs_controls':ratio,'core_primitive_uplift_stable':bsp,'combined_core_controls_uplift_stable':bsc,'core_uplift_stable_rate_ratio_vs_controls':stable_ratio,'per_core_template_primitive':per,'per_core_template_primitive_uplift_stable':per_stable,'total':total,'behavior_pair_count':_behavior_count(records),'behavior_pair_rate':br}
...
def run(args:argparse.Namespace,*,admission:Mapping[str,Any],authorization:Mapping[str,Any])->dict[str,Any]:
 ...
 fields=base._full_field_union(fresh,authority); input_binding=engine._self_hashed({'schema_version':'cn_program_primitive_market_successor_v2_input_binding_v1','repo_sha':repo_sha,'authorization_payload_sha256':authorization['authorization_payload_sha256'],'successor_plan_payload_sha256':plan['plan_payload_sha256'],'fresh_supply_payload_sha256':plan['fresh_supply']['payload_sha256'],'effective_spent_exact_count':7934,'fresh_exact_identities_sha256':plan['fresh_supply']['fresh_exact_identities_sha256'],'primitive_stats_payload_sha256':plan['search_authority']['primitive_stats_payload_sha256'],'production_results_in_primitive_stats':0,'v1_successor_results_in_primitive_stats':0,'source_v1_terminal_audit_payload_sha256':plan['source_v1_terminal_audit']['payload_sha256'],'source_v1_acceleration_full_parity_payload_sha256':plan['source_v1_acceleration_full_parity']['payload_sha256'],'acceleration_accuracy_audit_payload_sha256':plan['acceleration_accuracy_audit']['payload_sha256'],'field_columns_sha256':stable_hash(list(fields)),'evaluation_data_role':'DEVELOPMENT_ONLY','evaluator_pool_lifetime':'PERSISTENT_RUN_SCOPE','restricted_reads':{'validation':0,'holdout':0,'historical_2023':0,'forward_b':0,'forward_2026':0}},'input_binding_sha256'); large._write_json(root/'input_binding.json',input_binding)
