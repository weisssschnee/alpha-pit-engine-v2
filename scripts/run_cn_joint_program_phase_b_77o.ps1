...
if (-not $resolvedRoot.StartsWith(
    'D:\ChengboRemote\runtime\cn_joint_program_rolling_search_v0_phase_b_',
    [StringComparison]::OrdinalIgnoreCase
)) {
    throw "unexpected output root: $resolvedRoot"
}
if (Test-Path -LiteralPath $resolvedRoot) {
    throw "joint-program Phase B output root must be fresh: $resolvedRoot"
}
foreach ($path in @(
    $python,
    $git,
...
$stdoutPath = Join-Path $resolvedRoot 'joint_program_phase_b.stdout.log'
$stderrPath = Join-Path $resolvedRoot 'joint_program_phase_b.stderr.log'
[ordered]@{
    schema_version = 'cn_joint_program_phase_b_deployment_binding_v0'
    status = 'ACTIVE_DEVELOPMENT_ONLY_PILOT'
    repo_sha = $RepoSha
...
$leaseId = "validation-joint-program-phase-b-$PID"
...
try {
    ...
    & $python (Join-Path $resolvedRepo (
        'scripts\run_cn_joint_program_phase_b_v0.py'
    )) `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "joint-program Phase B failed: $LASTEXITCODE"
    }
    if (-not (Test-Path -LiteralPath (
        Join-Path $resolvedRoot 'CN_JOINT_PROGRAM_PHASE_B_COMPLETE.json'
    ))) {
        throw 'joint-program Phase B did not close'
    }
    $ErrorActionPreference = 'Stop'
} catch {
