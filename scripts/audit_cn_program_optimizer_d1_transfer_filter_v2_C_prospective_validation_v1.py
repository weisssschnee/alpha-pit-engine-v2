...
def audit(*,repo_root:Path,run_root:Path,authorization_path:Path,admission_path:Path,evidence_repo_sha:str,output:Path)->dict[str,Any]:
    ...
    if int(authorization.get("validation_reads_before_admission") if authorization.get("validation_reads_before_admission") is not None else -1)!=0 or any(int(authorization.get(key) or 0)!=0 for key in ("holdout_reads","historical_challenge_reads","forward_b_reads","forward_2026_reads")): raise RuntimeError("C validation authorization restricted-read drift")
    ...
    if prepared.get("zero_read_preflight_payload_sha256")!=zero_read_payload_sha or zero_read.get("implementation_repo_sha")!=evidence_repo_sha or int(zero_read.get("validation_reads_by_this_preflight") if zero_read.get("validation_reads_by_this_preflight") is not None else -1)!=0 or bool(zero_read.get("candidate_evaluation_executed")): raise RuntimeError("C validation zero-read boundary drift")
    if int(prepared.get("candidate_count") or 0)!=EXPECTED_CANDIDATE_COUNT or prepared.get("candidate_exact_identities_sha256")!=EXPECTED_CANDIDATE_SET_SHA256 or int(prepared.get("transfer_filter_selected_count") or 0)!=EXPECTED_SELECTED_COUNT or prepared.get("transfer_filter_selected_exact_identities_sha256")!=EXPECTED_SELECTED_SET_SHA256 or int(prepared.get("validation_reads_before_project_control_admission") if prepared.get("validation_reads_before_project_control_admission") is not None else -1)!=0: raise RuntimeError("C validation prepared membership drift")
    ...
    if input_binding.get("candidate_exact_identities_sha256")!=EXPECTED_CANDIDATE_SET_SHA256 or int(input_binding.get("candidate_count") or 0)!=EXPECTED_CANDIDATE_COUNT or input_binding.get("prepared_binding_payload_sha256")!=prepared_payload_sha or input_binding.get("zero_read_preflight_payload_sha256")!=zero_read_payload_sha or int(input_binding.get("validation_reads_before_project_control_admission") if input_binding.get("validation_reads_before_project_control_admission") is not None else -1)!=0: raise RuntimeError("C validation input binding drift")
