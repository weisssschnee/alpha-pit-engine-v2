...
def _split_audit(repo: Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "status": "PASS" if "shard-parallel CM requires --cm-split-manifest" in cp_text else "FAIL",
            "evidence": "parallel path fails when manifest is absent; serial fallback can still omit it",
            "path": str(cp.relative_to(repo)),
            "runtime_reachable": True,
        },
        {
            "component": "PHASE3CM_WORKER_LOCAL_SPLIT",
            "status": "FAIL_REACHABLE",
            "evidence": "worker evaluation calls _split_map(full_signal_times, ...) before fixed-manifest normalization",
            "path": f"{cm.relative_to(repo)}:2882",
            "runtime_reachable": "split_by_time = _split_map(full_signal_times" in cm_text,
        },
        {
            "component": "CHUNK04_RECOVERY_WORKERS",
            "status": "FAIL_REACHABLE",
            "evidence": "worker subprocess arguments omit --split-manifest; only the final exact merge receives it",
            "path": str(recovery.relative_to(repo)),
            "runtime_reachable": recovery_text.count("--split-manifest") == 1,
        },
        {
            "component": "FINAL_EXACT_NORMALIZATION",
            "status": "PASS_WITH_UPSTREAM_CAVEAT",
            "evidence": "fixed manifest is supplied to final exact merge and unknown manifest dates hard fail",
