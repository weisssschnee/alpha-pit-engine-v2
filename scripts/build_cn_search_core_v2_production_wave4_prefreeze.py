"""Freeze focused Search Core V2 Production Wave 4 before any new financial read."""
...
SCHEMA = "cn_search_core_v2_production_wave4_prefreeze_v1"
STATUS = "SEARCH_CORE_V2_PRODUCTION_WAVE4_PREFROZEN_BEFORE_FINANCIAL_READ"
...
WAVE3_EVIDENCE_FREEZE = Path("runtime/run_plans/cn_search_core_v2_production_wave3_evidence_freeze_20260826.json")
SOURCE_STATE = Path("runtime/run_plans/cn_search_core_v2_production_wave3_final_optimizer_state_79592cb_20260826.json")
FOCUS_REVIEW = Path("runtime/run_plans/cn_search_core_v2_wave4_focus_budget_review_20260826.json")
TEMPLATES = (
    "BASE_EVENT",
    "BASE_MARKET_EVENT",
    "BASE_TEMPORAL_EVENT",
...
HELD_TEMPLATES = ("BASE_MARKET", "BASE_TEMPORAL", "BASE_TEMPORAL_MARKET")
ROUNDS = 2
BATCH_SIZE = 24
...
def build(repo: Path, *, source_repo_sha: str) -> dict[str, Any]:
    ...
    freeze_path = repo / WAVE3_EVIDENCE_FREEZE
    ...
    audit_hash = _verify(audit, "audit_payload_sha256", "Wave3 postrun audit")
    freeze_hash = _verify(freeze, "evidence_freeze_payload_sha256", "Wave3 evidence freeze")
    state_hash = _verify(state, "snapshot_hash", "Wave3 final mature state")
    focus_hash = _verify(focus, "focus_review_payload_sha256", "Wave4 focus budget review")
    ...
    if (
        ...
        raise RuntimeError("Production Wave 4 policy authority drift")

    lineage = dict(audit.get("mature_state_lineage") or {})
    recommendation = dict(audit.get("project_control_recommendation") or {})
    if (
        audit.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE3_POSTRUN_AUDIT_COMPLETE_DEVELOPMENT_ARCHIVE_READY"
        ...
        raise RuntimeError("Production Wave 4 Wave3 audit authority drift")

    frozen_state = dict(freeze.get("artifacts", {}).get("final_optimizer_state") or {})
    if (
        freeze.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE3_EVIDENCE_FROZEN_DEVELOPMENT_ONLY"
        ...
        raise RuntimeError("Production Wave 4 Wave3 freeze authority drift")

    if (
        focus.get("status") != "SEARCH_CORE_V2_WAVE4_FOCUS_BUDGET_FROZEN_DEVELOPMENT_ONLY"
        ...
        raise RuntimeError("Production Wave 4 focus budget authority drift")
    ...
    implementation = {}
    for name, rel in {
        "runner": "scripts/run_cn_search_core_v2_production_wave4_v1.py",
    ...
    payload = {
        ...
        "source_wave3_evidence_freeze": {
            "relative_path": str(WAVE3_EVIDENCE_FREEZE).replace("\\", "/"),
            "file_sha256": _sha(freeze_path),
            "payload_sha256": freeze_hash,
        },
        "source_wave4_focus_budget_review": {
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runtime/run_plans/cn_search_core_v2_production_wave4_prefreeze_20260826.json"),
    )
        ...
        json.dumps(
            {
                ...
                "templates": payload["production_wave4"]["templates"],
                "total": payload["production_wave4"]["total_financial_evaluations"],
