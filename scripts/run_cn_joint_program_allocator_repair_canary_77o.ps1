...
if ($env:COMPUTERNAME -ne 'DESKTOP-77OPJ6F') {
    throw 'allocator-repair canary is authorized only on DESKTOP-77OPJ6F'
}
...
if (-not $resolvedRoot.StartsWith(
    'D:\ChengboRemote\runtime\cn_joint_program_allocator_repair_canary_',
    [StringComparison]::OrdinalIgnoreCase
)) {
    throw "unexpected output root: $resolvedRoot"
}
if (Test-Path -LiteralPath $resolvedRoot) {
    throw "allocator-repair output root must be fresh: $resolvedRoot"
}
foreach ($path in @(
    $python,
    $git,
...
if (
    ...
    throw 'allocator-repair resource profile drift'
}
if ($ExecutorWorkerCount -ne 10) {
    throw 'allocator-repair executor worker count is frozen at 10'
}
...
foreach ($path in @($freezeClosure, $fieldManifest, $priceManifest)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "immutable allocator-repair input missing: $path"
    }
}
...
$leaseId = "validation-joint-program-allocator-repair-$PID"
...
try {
    ...
    & $python (Join-Path $resolvedRepo (
        'scripts\run_cn_joint_program_allocator_repair_canary_v0.py'
    )) `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "allocator-repair canary failed: $LASTEXITCODE"
    }
    if (-not (Test-Path -LiteralPath (
        Join-Path $resolvedRoot 'CN_JOINT_PROGRAM_ALLOCATOR_REPAIR_CANARY_COMPLETE.json'
    ))) {
        throw 'allocator-repair canary did not close'
    }
    $ErrorActionPreference = 'Stop'
} catch {
