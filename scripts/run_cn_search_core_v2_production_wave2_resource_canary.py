"""Official zero-candidate resource canary for Production Wave 2."""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    supply = _read(args.production_wave2_supply_audit.resolve())
    ...
    if (
        ...
        raise RuntimeError("PRODUCTION_WAVE2_CANARY_SUPPLY_NOT_PASS")
    provisional = {
        "authorization_payload_sha256": stable_hash(
            {
                "role": "PRODUCTION_WAVE2_ZERO_FINANCIAL_CANARY",
    ...
    if not fields:
        raise RuntimeError("PRODUCTION_WAVE2_CANARY_FIELDS_EMPTY")
    ...
    if (
        ...
        raise RuntimeError("PRODUCTION_WAVE2_CANARY_FAIL")
    runner = repo / "scripts/run_cn_search_core_v2_production_wave2_v1.py"
    payload = {
        "schema_version": "cn_search_core_v2_production_wave2_resource_canary_v1",
        "status": "PASS",
