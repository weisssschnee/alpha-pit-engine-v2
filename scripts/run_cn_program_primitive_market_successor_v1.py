...
def verify_plan(path:Path)->dict[str,Any]:
 ...
 rc=dict(p['resource_contract']); aa=dict(p['acceleration_accuracy_audit']); g=dict(p['prospective_gate'])
 if rc.get('profile')!='SEARCH_DUAL_24' or int(rc.get('primary_executor_workers') or 0)!=24 or int(rc.get('fallback_executor_workers') or 0)!=16 or rc.get('evaluator_pool_lifetime')!='PERSISTENT_RUN_SCOPE' or float(rc.get('minimum_records_per_hour_after_first_checkpoint') or 0)<650.0 or int(rc.get('minimum_free_memory_bytes') or 0)<24*1024**3 or int(rc.get('throughput_enforcement_after_warm_checkpoints') or 0)!=2 or rc.get('persistent_pool_record_hash_parity_required') is not True or aa.get('status')!='PASS_SUCCESSOR_REAUTH_ELIGIBLE': raise RuntimeError('PRIMITIVE_MARKET_SUCCESSOR_RESOURCE_AUDIT_DRIFT')
