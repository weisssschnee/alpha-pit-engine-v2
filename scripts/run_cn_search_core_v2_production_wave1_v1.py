"""Single-arm mature State-Jump Generator production Wave 1 (development-only)."""
...
from our_system_phase2.services.program_search_core_policy_v2 import resolve_search_core_v2_primary_arm
...
def verify_prefreeze(path: Path) -> dict[str, Any]:
    payload = _read(path)
    _verify(payload, "prefreeze_payload_sha256", "Production Wave 1 prefreeze")
    wave = dict(payload.get("production_wave1") or {})
    mature = dict(payload.get("mature_state") or {})
    if (
        payload.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE1_PREFROZEN_BEFORE_FINANCIAL_READ"
        or wave.get("arm") != ARM
        ...
        raise RuntimeError("Production Wave 1 prefreeze contract drift")
    return payload
...
def _execution_plan() -> list[dict[str, Any]]:
    ...
    if len(rows) != CHECKPOINT_COUNT or sum(row["batch_size"] for row in rows) != TOTAL_EVALUATIONS:
        raise RuntimeError("Production Wave 1 execution plan drift")
    return rows
...
def run(args: argparse.Namespace, *, admission: Mapping[str, Any], authorization: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("PRODUCTION_WAVE1_ADMITTED_ROOT_NOT_CLEAN")
    repo = Path(__file__).resolve().parents[1]
    prefreeze = verify_prefreeze(args.production_wave1_prefreeze)
    ...
    if engine._sha256(state_path) != str(prefreeze["mature_state"]["file_sha256"]):
        raise RuntimeError("PRODUCTION_WAVE1_SOURCE_STATE_FILE_DRIFT")
    snapshot = _read(state_path)
    if str(snapshot.get("snapshot_hash") or "") != str(prefreeze["mature_state"]["payload_sha256"]):
        raise RuntimeError("PRODUCTION_WAVE1_SOURCE_STATE_PAYLOAD_DRIFT")
    policy = resolve_search_core_v2_primary_arm(snapshot)
    if policy["primary_arm"] != ARM or int(policy["development_observations"]) != SOURCE_OBSERVATIONS:
        raise RuntimeError("PRODUCTION_WAVE1_PRIMARY_POLICY_DRIFT")
    ...
    engine._write_json(
        root / "search_core_v2_production_wave1_policy_binding.json",
        {
            "schema_version": "cn_search_core_v2_production_wave1_policy_binding_v1",
    ...
    for step in _execution_plan():
        ...
        schedules = stage2._generated_schedules_batch(
            ...
            checkpoint_id=f"SEARCH_CORE_V2_PRODUCTION_WAVE1_R{round_index}_{template}",
        ...
        for schedule in schedules:
            schedule["production_wave_id"] = "SEARCH_CORE_V2_PRODUCTION_WAVE1_V1"
            schedule["production_round_index"] = round_index
            schedule["search_core_round_index"] = 6 + round_index
            schedule["search_core_stage"] = "PRODUCTION_WAVE1"
        ...
        for record in records:
            if any(
                ...
                raise RuntimeError("PRODUCTION_WAVE1_RESTRICTED_READ_DRIFT")
        ...
        for record in records:
            ...
            result["production_wave_id"] = "SEARCH_CORE_V2_PRODUCTION_WAVE1_V1"
            result["production_round_index"] = round_index
            result["search_core_stage"] = "PRODUCTION_WAVE1"
    ...
    if global_ordinal != TOTAL_EVALUATIONS or checkpoint_ordinal != CHECKPOINT_COUNT or len(results) != TOTAL_EVALUATIONS:
        raise RuntimeError("PRODUCTION_WAVE1_EXECUTION_COUNT_DRIFT")
    exacts = [str(row["exact_identity"]) for row in results]
    if len(set(exacts)) != TOTAL_EVALUATIONS:
        raise RuntimeError("PRODUCTION_WAVE1_EXACT_DUPLICATE")
    ...
    if (
        ...
        raise RuntimeError("PRODUCTION_WAVE1_MATURE_STATE_LINEAGE_DRIFT")
    engine._write_json(root / "FINAL_MATURE_OPTIMIZER_STATE.json", final_state)
    wall = time.perf_counter() - started
    closure = engine._self_hashed(
        {
            "schema_version": "cn_search_core_v2_production_wave1_complete_v1",
            "status": "SEARCH_CORE_V2_PRODUCTION_WAVE1_COMPLETE",
    ...
    engine._write_json(root / "CN_SEARCH_CORE_V2_PRODUCTION_WAVE1_COMPLETE.json", closure)
    return closure
