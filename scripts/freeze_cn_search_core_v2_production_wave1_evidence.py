"""Freeze audited Search Core V2 Production Wave 1 evidence into repo run-plans."""
...
STATUS = "SEARCH_CORE_V2_PRODUCTION_WAVE1_EVIDENCE_FROZEN_DEVELOPMENT_ONLY"
...
def freeze(*, repo: Path, output_root: Path, postrun_audit: Path, source_run_id: str) -> dict[str, Any]:
    ...
    if audit.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE1_POSTRUN_AUDIT_COMPLETE_ARCHIVE_READY":
        raise RuntimeError("Production Wave 1 postrun audit not frozen-ready")
    if audit["project_control_recommendation"].get("automatic_validation_authorized") is not False:
        raise RuntimeError("Production Wave 1 evidence freeze cannot carry validation authority")

    sources = {
        "terminal": output_root / "CN_SEARCH_CORE_V2_PRODUCTION_WAVE1_COMPLETE.json",
    ...
    final_state_hash = _verify(final_state, "snapshot_hash", "Production Wave 1 final state")
    ...
    suffix = "412d51c_20260825"
    targets = {
        "terminal": target_root / f"cn_search_core_v2_production_wave1_complete_{suffix}.json",
        "final_optimizer_state": target_root / f"cn_search_core_v2_production_wave1_final_optimizer_state_{suffix}.json",
        "productive_candidates": target_root / f"cn_search_core_v2_production_wave1_productive_candidates_{suffix}.jsonl",
        "stable_candidates": target_root / f"cn_search_core_v2_production_wave1_stable_candidates_{suffix}.jsonl",
    }
    if any(path.exists() for path in targets.values()):
        existing = [str(path) for path in targets.values() if path.exists()]
        raise FileExistsError(f"Production Wave 1 frozen evidence destination exists: {existing}")
    target_root.mkdir(parents=True, exist_ok=True)
    for key in sources:
        shutil.copyfile(sources[key], targets[key])
        if _sha(sources[key]) != _sha(targets[key]):
            raise RuntimeError(f"Production Wave 1 frozen copy drift: {key}")

    manifest = {
        "schema_version": "cn_search_core_v2_production_wave1_evidence_freeze_v1",
        "status": STATUS,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--manifest-output", type=Path, default=Path("runtime/run_plans/cn_search_core_v2_production_wave1_evidence_freeze_20260825.json"))
