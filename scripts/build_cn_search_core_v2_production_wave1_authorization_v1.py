"""Build Production Wave 1 authorization after zero-financial supply/canary."""
...
from our_system_phase2.runtime import cn_search_core_v2_production_wave1_v1 as runtime
...
def build(repo: Path, *, canary: Path, supply_audit: Path) -> dict[str, Any]:
    ...
    supply_hash = _verify(supply, "audit_payload_sha256", "Production Wave 1 supply")
    canary_path = canary.resolve()
    canary_payload = _read(canary_path)
    canary_hash = _verify(canary_payload, "official_canary_payload_sha256", "Production Wave 1 canary")
    runner = repo / "scripts/run_cn_search_core_v2_production_wave1_v1.py"
    ...
    if (
        ...
        raise RuntimeError("Production Wave 1 policy not approved")
    if (
        prefreeze.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE1_PREFROZEN_BEFORE_FINANCIAL_READ"
        or int(prefreeze["production_wave1"]["total_financial_evaluations"]) != runtime.TOTAL_EVALUATIONS
        or bool(prefreeze.get("financial_labels_read_by_builder"))
    ):
        raise RuntimeError("Production Wave 1 prefreeze drift")
    if (
        ...
        raise RuntimeError("Production Wave 1 supply drift")
    if (
        ...
        raise RuntimeError("Production Wave 1 canary drift")
    payload = {
        "schema_version": runtime.AUTHORIZATION_SCHEMA,
        "status": "SEARCH_CORE_V2_PRODUCTION_WAVE1_AUTHORIZED_NOT_RUN",
        ...
        "production_contract": dict(prefreeze["production_wave1"]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = build(args.repo_root, canary=args.official_resource_canary, supply_audit=args.production_wave1_supply_audit)
