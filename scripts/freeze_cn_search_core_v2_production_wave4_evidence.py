"""Freeze audited Search Core V2 Production Wave 4 evidence into repo run-plans."""
...
STATUS = "SEARCH_CORE_V2_PRODUCTION_WAVE4_EVIDENCE_FROZEN_DEVELOPMENT_ONLY"
...
def freeze(*, repo: Path, output_root: Path, postrun_audit: Path, source_run_id: str) -> dict[str, Any]:
    ...
    audit_hash = _verify(audit, "audit_payload_sha256", "Production Wave 4 postrun audit")
    if audit.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE4_POSTRUN_AUDIT_COMPLETE_DEVELOPMENT_ARCHIVE_READY":
        raise RuntimeError("Production Wave 4 postrun audit not frozen-ready")
    if audit["project_control_recommendation"].get("automatic_validation_authorized") is not False:
        raise RuntimeError("Production Wave 4 evidence freeze cannot carry validation authority")

    sources = {
        "terminal": output_root / "CN_SEARCH_CORE_V2_PRODUCTION_WAVE4_COMPLETE.json",
    ...
    final_state_hash = _verify(final_state, "snapshot_hash", "Production Wave 4 final state")
    ...
    suffix = "3eb0b15_20260826"
    targets = {
        "terminal": target_root / f"cn_search_core_v2_production_wave4_complete_{suffix}.json",
        "final_optimizer_state": target_root / f"cn_search_core_v2_production_wave4_final_optimizer_state_{suffix}.json",
        "productive_candidates": target_root / f"cn_search_core_v2_production_wave4_productive_candidates_{suffix}.jsonl",
        "stable_candidates": target_root / f"cn_search_core_v2_production_wave4_stable_candidates_{suffix}.jsonl",
        "pair_native_challenger_candidates": target_root / f"cn_search_core_v2_production_wave4_pair_native_challenger_candidates_{suffix}.jsonl",
    }
    if any(path.exists() for path in targets.values()):
        existing = [str(path) for path in targets.values() if path.exists()]
        raise FileExistsError(f"Production Wave 4 frozen evidence destination exists: {existing}")
    target_root.mkdir(parents=True, exist_ok=True)
    for key in sources:
        shutil.copyfile(sources[key], targets[key])
        if _sha(sources[key]) != _sha(targets[key]):
            raise RuntimeError(f"Production Wave 4 frozen copy drift: {key}")

    manifest = {
        "schema_version": "cn_search_core_v2_production_wave4_evidence_freeze_v1",
        "status": STATUS,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--manifest-output", type=Path, default=Path("runtime/run_plans/cn_search_core_v2_production_wave4_evidence_freeze_20260826.json"))
