...
if (-not $resolvedRoot.StartsWith(
    'D:\ChengboRemote\runtime\cn_hybrid_search_productivity_medium_',
    [StringComparison]::OrdinalIgnoreCase
)) {
...
[ordered]@{
    ...
    campaign_profile = 'cn_hybrid_search_productivity_medium_v1'
    formal_asks_per_checkpoint = 384
    maximum_checkpoints = 8
    maximum_formal_fresh_exact_asks = 3072
    policy_arm_ratio = [ordered]@{
        HYBRID_TPE_AVAILABILITY = 0.5
        AVAILABILITY_AWARE_UNIFORM = 0.5
    }
...
$campaignArgs = @(
    ...
    (Join-Path $resolvedRepo 'runtime\run_plans\cn_hybrid_search_productivity_medium_v1_authorization.json'),
    '--registry',
