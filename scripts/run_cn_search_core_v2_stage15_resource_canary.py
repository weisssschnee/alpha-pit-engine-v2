"""Official zero-financial resource canary for Search Core V2 Stage-1.5."""
...
def run(args:argparse.Namespace)->dict[str,Any]:
    ...
    supply=_read(args.mature_state_supply_audit.resolve()); supply_hash=_verify(supply,"audit_payload_sha256","Stage-1.5 mature supply")
    if (
    ...
    ): raise RuntimeError("STAGE15_CANARY_SUPPLY_NOT_PASS")
    provisional={
        "authorization_payload_sha256":stable_hash({"role":"SEARCH_CORE_V2_STAGE15_ZERO_FINANCIAL_CANARY","prefreeze":pre["prefreeze_payload_sha256"],"supply":supply_hash}),
    ...
    for checkpoint_ordinal,template in enumerate(stage1.TEMPLATES):
        ...
        if len(exacts)!=24: raise RuntimeError("STAGE15_CANARY_ARM_A_UNDERFILL")
        for template_ordinal,exact in enumerate(exacts):
            schedules.append(stage15._static_schedule(by_exact[exact],authority=authority,global_ordinal=global_ordinal,checkpoint_ordinal=checkpoint_ordinal,template_ordinal=template_ordinal)); global_ordinal+=1
    if len(schedules)!=168: raise RuntimeError("STAGE15_CANARY_ARM_A_COUNT_DRIFT")
    ...
    if not fields: raise RuntimeError("STAGE15_CANARY_FIELD_SURFACE_EMPTY")
    ...
    ): raise RuntimeError("STAGE15_OFFICIAL_RESOURCE_CANARY_FAIL")
    runner=repo/"scripts/run_cn_search_core_v2_stage15_v1.py"
    payload={
        "schema_version":"cn_search_core_v2_stage15_official_resource_canary_v1","status":"PASS","repo_sha":str(args.repo_sha),
...
def main(argv:list[str]|None=None)->int:
    ...
    p.add_argument("--source-stage-d-authorization",type=Path,required=True); p.add_argument("--stage15-prefreeze",type=Path,required=True)
