...
ROUTE_ID='cn-program-primitive-market-successor-v1'
CAMPAIGN_ID='CN_PROGRAM_PRIMITIVE_MARKET_SUCCESSOR_V1'
CAMPAIGN_PROFILE='cn_program_primitive_market_successor_v1'
AUTHORIZATION_SCHEMA='cn_program_primitive_market_successor_authorization_v1'
AUTHORIZATION_RELATIVE_PATH=Path('runtime/run_plans/cn_program_primitive_market_successor_authorization_v1.json')
...
def verify_authorization(path:Path,*,repo_root:Path|None=None)->dict[str,Any]:
 ...
 if int(plan['budget']['hard_cap_logical_records'])!=360 or plan['search_authority']['production_feedback_imported_into_primitive_stats'] is not False: raise ValueError('successor plan contract drift')
 rc=dict(p['resource_contract'])
 if rc.get('profile')!='SEARCH_DUAL_24' or int(rc.get('cpu_threads') or 0)!=24 or int(rc.get('primary_executor_workers') or 0)!=24 or int(rc.get('hard_cap_logical_records') or 0)!=360 or int(rc.get('field_column_count') or 0)!=53: raise ValueError('market successor resource contract drift')
...
def main(argv:Sequence[str]|None=None)->int:
 ...
 from scripts.run_cn_program_primitive_market_successor_v1 import run
