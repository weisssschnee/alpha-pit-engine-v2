"""Project-Control route for Search Core V2 Production Wave 2."""
...
ROUTE_ID = "cn-search-core-v2-production-wave2-v1"
CAMPAIGN_ID = "CN_SEARCH_CORE_V2_PRODUCTION_WAVE2_V1"
CAMPAIGN_PROFILE = "cn_search_core_v2_production_wave2_v1"
AUTHORIZATION_SCHEMA = "cn_search_core_v2_production_wave2_authorization_v1"
AUTHORIZATION_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_production_wave2_authorization_v1.json")
PREFREEZE_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_production_wave2_prefreeze_20260825.json")
SUPPLY_AUDIT_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_production_wave2_supply_audit_20260825.json")
...
def verify_authorization(path: Path, *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if not claimed or stable_hash(body) != claimed:
        raise ValueError("Production Wave 2 authorization self-hash drift")
    if (
        payload.get("schema_version") != AUTHORIZATION_SCHEMA
        or payload.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE2_AUTHORIZED_NOT_RUN"
        ...
        raise ValueError("Production Wave 2 authorization contract drift")
    ...
    if sha256_file(source_path) != str(source["file_sha256"]):
        raise ValueError("Production Wave 2 source Stage-D file drift")
    ...
    if source_auth.get("authorization_payload_sha256") != source["payload_sha256"]:
        raise ValueError("Production Wave 2 source Stage-D payload drift")
    policy = _bound(root, payload["source_policy_review"], field="policy_review_payload_sha256", label="Production Wave 2 policy review")
    prefreeze = _bound(root, payload["production_wave2_prefreeze"], field="prefreeze_payload_sha256", label="Production Wave 2 prefreeze")
    supply = _bound(root, payload["production_wave2_supply_audit"], field="audit_payload_sha256", label="Production Wave 2 supply audit")
    ...
    if pair_contract != dict(prefreeze.get("pair_native_annotation_contract") or {}):
        raise ValueError("Production Wave 2 pair-native authorization binding drift")
    if (
        ...
        raise ValueError("Production Wave 2 pair-native authorization contract drift")
    if (
        ...
        or int(prefreeze["production_wave2"].get("total_financial_evaluations") or 0) != TOTAL_EVALUATIONS
        or int(prefreeze["production_wave2"].get("total_checkpoint_count") or 0) != CHECKPOINT_COUNT
        or supply.get("status") != "PASS_ZERO_FINANCIAL_PRODUCTION_WAVE2_CHECKPOINT_SUPPLY_AUDIT"
        ...
        raise ValueError("Production Wave 2 authority evidence drift")
    ...
    if (
        ...
        raise ValueError("Production Wave 2 resource contract drift")
    if any(int(value) != 0 for value in dict(payload["restricted_reads"]).values()):
        raise ValueError("Production Wave 2 restricted-read drift")
    return payload
...
def verify_official_canary(path: Path, authorization: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    if observed != str(binding["file_sha256"]):
        raise ProjectControlDenied("Production Wave 2 canary file drift")
    ...
    if (
        ...
        raise ProjectControlDenied("Production Wave 2 canary payload drift")
    runner = repo_root / "scripts/run_cn_search_core_v2_production_wave2_v1.py"
    if sha256_file(runner) != str(binding["runner_source_file_sha256"]):
        raise ProjectControlDenied("Production Wave 2 runner changed after canary")
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--production-wave2-prefreeze", type=Path, required=True)
    parser.add_argument("--production-wave2-supply-audit", type=Path, required=True)
    ...
    if dict(verified.payload) != authorization:
        raise ProjectControlDenied("Production Wave 2 authorization payload drift")
    root = Path(__file__).resolve().parents[3]
    expected_pre = (root / Path(str(authorization["production_wave2_prefreeze"]["relative_path"]))).resolve()
    expected_supply = (root / Path(str(authorization["production_wave2_supply_audit"]["relative_path"]))).resolve()
    if args.production_wave2_prefreeze.resolve() != expected_pre or args.production_wave2_supply_audit.resolve() != expected_supply:
        raise ProjectControlDenied("Production Wave 2 frozen path outside authorization")
    ...
    from scripts.run_cn_search_core_v2_production_wave2_v1 import run
