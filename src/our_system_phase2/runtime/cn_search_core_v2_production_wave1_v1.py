"""Project-Control route for Search Core V2 Production Wave 1."""
...
ROUTE_ID = "cn-search-core-v2-production-wave1-v1"
CAMPAIGN_ID = "CN_SEARCH_CORE_V2_PRODUCTION_WAVE1_V1"
CAMPAIGN_PROFILE = "cn_search_core_v2_production_wave1_v1"
AUTHORIZATION_SCHEMA = "cn_search_core_v2_production_wave1_authorization_v1"
AUTHORIZATION_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_production_wave1_authorization_v1.json")
PREFREEZE_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_production_wave1_prefreeze_20260825.json")
SUPPLY_AUDIT_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_production_wave1_supply_audit_20260825.json")
...
def verify_authorization(path: Path, *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if not claimed or stable_hash(body) != claimed:
        raise ValueError("Production Wave 1 authorization self-hash drift")
    if (
        payload.get("schema_version") != AUTHORIZATION_SCHEMA
        or payload.get("status") != "SEARCH_CORE_V2_PRODUCTION_WAVE1_AUTHORIZED_NOT_RUN"
        ...
        raise ValueError("Production Wave 1 authorization contract drift")
    ...
    if sha256_file(source_path) != str(source["file_sha256"]):
        raise ValueError("Production Wave 1 source Stage-D file drift")
    ...
    if source_auth.get("authorization_payload_sha256") != source["payload_sha256"]:
        raise ValueError("Production Wave 1 source Stage-D payload drift")
    policy = _bound(root, payload["source_policy_review"], field="policy_review_payload_sha256", label="Production Wave 1 policy review")
    prefreeze = _bound(root, payload["production_wave1_prefreeze"], field="prefreeze_payload_sha256", label="Production Wave 1 prefreeze")
    supply = _bound(root, payload["production_wave1_supply_audit"], field="audit_payload_sha256", label="Production Wave 1 supply audit")
    if (
        ...
        or int(prefreeze["production_wave1"].get("total_financial_evaluations") or 0) != TOTAL_EVALUATIONS
        or int(prefreeze["production_wave1"].get("total_checkpoint_count") or 0) != CHECKPOINT_COUNT
        or supply.get("status") != "PASS_ZERO_FINANCIAL_PRODUCTION_WAVE1_CHECKPOINT_SUPPLY_AUDIT"
        ...
        raise ValueError("Production Wave 1 authority evidence drift")
    ...
    if (
        ...
        raise ValueError("Production Wave 1 resource contract drift")
    if any(int(value) != 0 for value in dict(payload["restricted_reads"]).values()):
        raise ValueError("Production Wave 1 restricted-read drift")
    return payload
...
def verify_official_canary(path: Path, authorization: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    if observed != str(binding["file_sha256"]):
        raise ProjectControlDenied("Production Wave 1 canary file drift")
    ...
    if (
        ...
        raise ProjectControlDenied("Production Wave 1 canary payload drift")
    runner = repo_root / "scripts/run_cn_search_core_v2_production_wave1_v1.py"
    if sha256_file(runner) != str(binding["runner_source_file_sha256"]):
        raise ProjectControlDenied("Production Wave 1 runner changed after canary")
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--production-wave1-prefreeze", type=Path, required=True)
    parser.add_argument("--production-wave1-supply-audit", type=Path, required=True)
    ...
    if dict(verified.payload) != authorization:
        raise ProjectControlDenied("Production Wave 1 authorization payload drift")
    root = Path(__file__).resolve().parents[3]
    expected_pre = (root / Path(str(authorization["production_wave1_prefreeze"]["relative_path"]))).resolve()
    expected_supply = (root / Path(str(authorization["production_wave1_supply_audit"]["relative_path"]))).resolve()
    if args.production_wave1_prefreeze.resolve() != expected_pre or args.production_wave1_supply_audit.resolve() != expected_supply:
        raise ProjectControlDenied("Production Wave 1 frozen path outside authorization")
    ...
    from scripts.run_cn_search_core_v2_production_wave1_v1 import run
