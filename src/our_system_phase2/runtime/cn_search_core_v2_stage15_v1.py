"""Project-Control route for Search Core V2 Stage-1.5 mature-state confirmation."""
...
ROUTE_ID = "cn-search-core-v2-stage15-v1"
CAMPAIGN_ID = "CN_SEARCH_CORE_V2_STAGE15_V1"
CAMPAIGN_PROFILE = "cn_search_core_v2_stage15_v1"
AUTHORIZATION_SCHEMA = "cn_search_core_v2_stage15_authorization_v1"
AUTHORIZATION_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_stage15_authorization_v1.json")
PREFREEZE_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_stage15_prefreeze_20260824.json")
SUPPLY_AUDIT_RELATIVE_PATH = Path("runtime/run_plans/cn_search_core_v2_stage15_mature_supply_audit_20260824.json")
...
def verify_authorization(path: Path, *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if not claimed or stable_hash(body) != claimed:
        raise ValueError("Stage-1.5 authorization self-hash drift")
    if (
        payload.get("schema_version") != AUTHORIZATION_SCHEMA
        or payload.get("status") != "SEARCH_CORE_V2_STAGE15_AUTHORIZED_NOT_RUN"
        ...
        raise ValueError("Stage-1.5 authorization contract drift")
    ...
    if sha256_file(source_path) != str(source["file_sha256"]):
        raise ValueError("Stage-1.5 source Stage-D authorization file drift")
    ...
    if source_auth.get("authorization_payload_sha256") != source["payload_sha256"]:
        raise ValueError("Stage-1.5 source Stage-D authorization payload drift")
    pre = _bound(root, payload["stage15_prefreeze"], payload_field="prefreeze_payload_sha256", label="Stage-1.5 prefreeze")
    if pre.get("status") != "SEARCH_CORE_V2_STAGE15_PREFROZEN_BEFORE_FINANCIAL_READ" or int(pre["stage15"]["total_financial_evaluations"]) != TOTAL_EVALUATIONS:
        raise ValueError("Stage-1.5 prefreeze binding drift")
    supply = _bound(root, payload["mature_state_supply_audit"], payload_field="audit_payload_sha256", label="Stage-1.5 mature supply audit")
    if (
        supply.get("status") != "PASS_ZERO_FINANCIAL_MATURE_STATE_JUMP_SUPPLY_AUDIT"
        ...
        raise ValueError("Stage-1.5 mature supply evidence drift")
    ...
    if (
        ...
        raise ValueError("Stage-1.5 resource contract drift")
    if any(int(v) != 0 for v in dict(payload["restricted_reads"]).values()):
        raise ValueError("Stage-1.5 restricted-read authorization drift")
    return payload
...
def verify_official_canary(path: Path, authorization: Mapping[str, Any], *, repo_root: Path) -> dict[str, Any]:
    ...
    if observed != str(binding["file_sha256"]):
        raise ProjectControlDenied("Stage-1.5 official canary file drift")
    ...
    if (
        ...
        raise ProjectControlDenied("Stage-1.5 official canary payload drift")
    runner = repo_root / "scripts/run_cn_search_core_v2_stage15_v1.py"
    if sha256_file(runner) != str(binding["runner_source_file_sha256"]):
        raise ProjectControlDenied("Stage-1.5 runner changed after canary")
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if dict(verified.payload) != authorization:
        raise ProjectControlDenied("Stage-1.5 authorization payload drift")
    root = Path(__file__).resolve().parents[3]
    expected_pre = (root / Path(str(authorization["stage15_prefreeze"]["relative_path"]))).resolve()
    expected_supply = (root / Path(str(authorization["mature_state_supply_audit"]["relative_path"]))).resolve()
    if args.stage15_prefreeze.resolve() != expected_pre:
        raise ProjectControlDenied("Stage-1.5 prefreeze path outside authorization")
    if args.mature_state_supply_audit.resolve() != expected_supply:
        raise ProjectControlDenied("Stage-1.5 supply-audit path outside authorization")
    ...
    from scripts.run_cn_search_core_v2_stage15_v1 import run
