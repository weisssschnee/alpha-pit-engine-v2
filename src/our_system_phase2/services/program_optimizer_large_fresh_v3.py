"""Three-arm Large Fresh V3 state with Primitive-local as the primary searcher.

V3 preserves the V2 Uniform reserve and Catalog Typed Evolution challenger, but
assigns the majority of template checkpoints to the independently confirmed
hierarchical primitive-credit adapter.
"""
...
STATE_SCHEMA = "cn_program_optimizer_large_fresh_bandit_v3"
ARMS = (
    UNIFORM_CONTROL,
    PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1,
    CATALOG_TYPED_EVOLUTION_PROGRAM_V2,
...
class LargeFreshProgramBanditV3:
    """Replayable three-arm state: Primitive primary, Uniform reserve, Evolution challenger."""
    ...
    def __init__(
        ...
        *,
        ...
        if set(entries_by_arm) != set(ARMS) or set(seeds) != set(ARMS):
            raise ValueError("LARGE_FRESH_V3_ARM_COVERAGE_DRIFT")
        ...
        if len(hashes) != 1:
            raise ValueError("LARGE_FRESH_V3_COMMON_SPACE_DRIFT")
        self.program_space_hash = next(iter(hashes))
        self.seeds = {arm: int(seeds[arm]) for arm in ARMS}
        self.primitive_config = dict(primitive_config)
        self.evolution_config = dict(evolution_config)
        self.primitive_config_contract = {
            "primitive_stats_payload_sha256": str(
                self.primitive_config.get("primitive_stats_payload_sha256") or ""
            ),
            "metadata_payload_sha256": stable_hash(
                dict(
                    sorted(
                        dict(
                            self.primitive_config.get("metadata_by_exact_identity")
                            or {}
                        ).items()
                    )
                )
        ...
        if not self.primitive_config_contract["primitive_stats_payload_sha256"]:
            raise ValueError("LARGE_FRESH_V3_PRIMITIVE_STATS_AUTHORITY_MISSING")
        self.adapters = {
            ...
            PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1: PrimitiveLocalProgramSearchAdapterV1(
                entries=self.entries_by_arm[PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1],
                seen_exact_identities=(),
    ...
    def _ask_live(
        ...
        *,
        ...
        if adapter is None:
            raise ValueError(f"LARGE_FRESH_V3_ARM_UNKNOWN:{arm}")
        return adapter.ask(
    ...
    def commit_ask(
        ...
        **kwargs: Any,
        ...
        if committed != [dict(row) for row in expected_asks]:
            raise RuntimeError("LARGE_FRESH_V3_PREVIEW_COMMIT_DRIFT")
    ...
    def discard_nonlearning_pending(
        ...
        if list(adapter._pending.values()) != [dict(row) for row in expected_asks]:
            raise RuntimeError("LARGE_FRESH_V3_PENDING_DRIFT")
        adapter._pending.clear()
        return {
            "schema_version": "cn_program_large_fresh_v3_nonlearning_discard_v1",
    ...
    def snapshot(self) -> dict[str, Any]:
        payload = {
            ...
            "primitive_config_contract": dict(self.primitive_config_contract),
            "evolution_config": dict(self.evolution_config),
            "arms": {arm: self.adapters[arm].snapshot() for arm in ARMS},
            "observations": self.observations,
            "primary_arm": PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1,
            "uniform_reserve_retained": True,
            "typed_evolution_challenger_retained": True,
            "stage_c_or_stage_d_financial_observations_imported": False,
        }
    ...
    def restore(
        ...
        *,
        ...
        if not claimed or stable_hash(payload) != claimed:
            raise ValueError("LARGE_FRESH_V3_STATE_SELF_HASH_DRIFT")
        if payload.get("schema_version") != STATE_SCHEMA:
            raise ValueError("LARGE_FRESH_V3_STATE_SCHEMA_DRIFT")
        if expected_campaign_id is not None and str(payload["campaign_id"]) != str(
            expected_campaign_id
        ):
            raise ValueError("LARGE_FRESH_V3_CAMPAIGN_DRIFT")
        state = cls(
        ...
        if state.program_space_hash != str(payload["program_space_hash"]):
            raise ValueError("LARGE_FRESH_V3_COMMON_SPACE_DRIFT")
        if state.primitive_config_contract != dict(payload["primitive_config_contract"]):
            raise ValueError("LARGE_FRESH_V3_PRIMITIVE_CONFIG_DRIFT")
        state.adapters = {
            ...
            PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1: PrimitiveLocalProgramSearchAdapterV1.restore(
                snapshot=dict(
                    payload["arms"][PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1]
                ),
                entries=state.entries_by_arm[PRIMITIVE_LOCAL_HIERARCHICAL_PROGRAM_V1],
                seen_exact_identities=(),
        ...
        if state.snapshot() != dict(snapshot):
            raise ValueError("LARGE_FRESH_V3_STATE_REPLAY_DRIFT")
        return state
...
__all__ = ["ARMS", "STATE_SCHEMA", "LargeFreshProgramBanditV3"]
