...
| First B1S CANARY invalidated | Mixed-role physical row groups were filtered after read | Run retained as diagnostic only; no research decision may rely on it |
| Physical development release established | Logical train filters could still open validation/holdout pages | 16-shard release contains only 446,443,583 development rows; read ledger forbidden counters are zero |
...
shard-local split repair
  ...
  -> group-aware strict-priority selector and event/state support repair
  -> two-seed Repair Capability CANARY at f4fc41d
